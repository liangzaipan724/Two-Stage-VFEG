...
from datasets_mead import MEAD_test
import random
...
BATCH_SIZE = 10
root_dir = '/home/main/'
data_dir = "/data/HDD1/mead/"
GPU = "2"
postfix = "-j-sl-random-of"   #postfix 后缀
...
def main():
    ...
    model = FlowDiffusion(is_train=True,
                          ...
                          pretrained_pth="/data/HDD1/mead/MEAD.pth")
    model.cuda()
    ...
    setup_seed(args.random_seed)
    testloader = data.DataLoader(MEAD_test(data_dir=data_dir,
    ...
    for i_iter, batch in enumerate(testloader):
        ...
        for batch_idx in range(bs):
            # save one video
            msk_size = ref_imgs.shape[-1]#获取ref_imgs张量的最后一个维度的大小，将其赋值给msk_size。这里假设最后一个维度表示图像的大小或遮罩尺寸。
