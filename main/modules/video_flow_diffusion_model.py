...
from stage_one.modules.generator import Generator
from stage_one.modules.bg_motion_predictor import BGMotionPredictor
from stage_one.modules.region_predictor import RegionPredictor
from modules.video_flow_diffusion import Unet3D, GaussianDiffusion
import yaml
...
class FlowDiffusion(nn.Module):
    def __init__(self, img_size=32, num_frames=40, sampling_timesteps=250,
        ...
        self.unet = Unet3D(dim=64,
                           channels=3 + 256,   #文本数据编码为一个长度为 256 的特征向量，然后将其与图像数据进行连接，
                           out_grid_dim=2,
                           out_conf_dim=1,
                           dim_mults=dim_mults,
    ...
    def forward(self):
        ...
        with torch.no_grad():
            if self.ref_img.dtype == torch.uint8:
                self.ref_img = self.ref_img.float()
...
if __name__ == "__main__":
    ...
    model = FlowDiffusion(use_residual_flow=False,
                          ...
                          pretrained_pth="/data/PRETRAINED.pth")
    model.cuda()
    model.eval()
