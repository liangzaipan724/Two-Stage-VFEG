...
from modules.text import tokenize, bert_embed, BERT_MODEL_DIM
...
class Attention(nn.Module):
    def __init__(
        ...
        self.to_qkv = nn.Linear(dim, hidden_dim * 3, bias=False)     #一个线性层，将输入特征转换为查询（query）、键（key）和值（value）的表示。该线性层的输出维度为 hidden_dim * 3。
    ...
    def forward(
        ...
        n, device = x.shape[-2], x.device  #获取输入信息x
